...
def process_pengembalian(buku, mahasiswa):
    ...
    def kembalikan_buku(nama, judul):
        ...
        elif judul in mahasiswa[nama].get("pinjaman", []) == mahasiswa[nama].get("ingin_kembali", []):
            if len(mahasiswa[nama].get("pinjaman", [])) < 1:
                for i in range(2):
                    mahasiswa[nama].update({"pinjaman":[]})
                buku[judul].update({"stok": +1})
                buku[judul].update({"status": "Tersedia"})
            return (judul, "Berhasil", None)
